...
\- Tìm kiếm từ 2 ký tự trở lên; hỗ trợ tiếng Việt có dấu/không dấu bằng PostgreSQL Full-Text Search, `unaccent`, `pg\_trgm` và GIN index.
...
\- Nutrition lưu trong các cột `Nutrition\_\*` của bảng `Recipes`, không có bảng hoặc ID riêng.
...
| `RecipeNutrition` | Nhóm cột nullable `Nutrition\_\*` trong `Recipes`; gồm calories, protein, carbohydrates, fat, fiber, sodium |
...
\- \[PDF đặc tả gốc](./SRS\_Culinary\_Blog\_v1.0.0.pdf): nguồn phạm vi chức năng và công nghệ.
